...
{    
    ...
    public class VRSL_LocalUIControlPanel : UdonSharpBehaviour
    {
       ...
       public bool sperateInGameInterpolationSpeed = true;
...
#if TEXTMESHPRO
        private void SafeApplyText(TextMeshProUGUI text, string value)
        {
            if (text == null) return;
            text.text = value;
        }
        ...
        public void _Test()
        {
            if (outputDebugLogs) Debug.Log("This is a test");
        }

        public void SafeSetButtonInteractable(UnityEngine.UI.Button button, bool interactable)
        {
            ...
            UnityEngine.UI.Button buttonHigh, UnityEngine.UI.Button buttonMedium, UnityEngine.UI.Button buttonLow, VolumetricQualityModes quality)
        {
            ...
            SafeSetButtonInteractable(buttonMedium, quality != VolumetricQualityModes.Medium);
            SafeSetButtonInteractable(buttonLow, quality != VolumetricQualityModes.Low);
        }
        ...
        {
            // Safe update UI buttons
            SafeSetButtonInteractable(strobeHighButton, strobeLowButton, strobeQuality);
        }
...
#if TEXTMESHPRO
            SafeApplyText(fixtureSliderTextPro, text);
#endif
        }
...
#if TEXTMESHPRO
            SafeApplyText(volumetricSliderTextPro, text);
#endif
        }
...
#if TEXTMESHPRO
            SafeApplyText(projectionSliderTextPro, text);
#endif
        }
...
#if TEXTMESHPRO
            SafeApplyText(discoBallSliderTextPro, text);
#endif
        }
        ...
        public void _SetBloomIntensity()
        {
            if (bloomAnimator != null)
            {
                ...
                string text = Mathf.Round(bloomSlider.value * 100.0f).ToString();
                SafeApplyText(bloomSliderText, text);
#if TEXTMESHPRO
                SafeApplyText(bloomSliderTextPro, text);
#endif
            }
            else
            {
